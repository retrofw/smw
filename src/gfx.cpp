...
bool gfx_init(int w, int h, bool fullscreen)
{
    ...
    if(fullscreen){
			real_screen = SDL_SetVideoMode(w, h, GFX_BPP, SDL_HWSURFACE | SDL_TRIPLEBUF | SDL_FULLSCREEN);
			// ScreenSurface = SDL_SetVideoMode(320, 480, 16, SDL_HWSURFACE);
			// real_screen = SDL_CreateRGBSurface(SDL_SWSURFACE, 320, 240, GFX_BPP, 0, 0, 0, 0);
		}
		else{
			real_screen = SDL_SetVideoMode(w, h, GFX_BPP, SDL_HWSURFACE | SDL_TRIPLEBUF);
			// ScreenSurface = SDL_SetVideoMode(320, 480, 16, SDL_HWSURFACE);
			// real_screen = SDL_CreateRGBSurface(SDL_SWSURFACE, 320, 240, GFX_BPP, 0, 0, 0, 0);
		}
    ...
    SDL_Surface* temp = SDL_CreateRGBSurface(SDL_SWSURFACE,w,h,GFX_BPP,0,0,0,0);
    screen = SDL_DisplayFormat(temp);
    SDL_FreeSurface(temp);

    if ( screen == NULL || real_screen == NULL ) 
	{
...
#else
      // real_screen = SDL_SetVideoMode(320, 240, GFX_BPP, flags);
      real_screen = SDL_SetVideoMode(w, h, GFX_BPP, SDL_HWSURFACE | SDL_TRIPLEBUF);
...
SDL_Surface * gfx_createskinsurface(SDL_Surface * skin, short spriteindex, Uint8 r, Uint8 g, Uint8 b, short colorScheme, bool expand, bool reverse)
{
	...
	if( SDL_SetColorKey(temp, COLORKEY_FLAGS, SDL_MapRGB(temp->format, r, g, b)) < 0)
	{
...
bool gfxSprite::init(const std::string& filename, Uint8 r, Uint8 g, Uint8 b)
{
	...
	if( SDL_SetColorKey(m_picture, COLORKEY_FLAGS, SDL_MapRGB(m_picture->format, r, g, b)) < 0)
	{
...
bool gfxSprite::init(const std::string& filename, Uint8 r, Uint8 g, Uint8 b, Uint8 a)
{
	...
	if( (SDL_SetAlpha(m_picture, ALPHA_FLAGS, a)) < 0)
	{
...
{
	if( (SDL_SetAlpha(m_picture, ALPHA_FLAGS, alpha)) < 0)
	{
...
{
	if( (SDL_SetAlpha(m_font->Surface, ALPHA_FLAGS, alpha)) < 0)
	{
		printf("\n ERROR: couldn't set alpha on sprite: %s\n", SDL_GetError());
	}
}
